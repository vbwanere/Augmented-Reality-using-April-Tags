...
# setting paths of the files to be used:
curr_file_path = os.path.abspath(__file__) # get current file path
curr_dir = os.path.dirname(curr_file_path) # get current file directory
...
if __name__ == "__main__":
    ...
    if args.click_points:
        ...
        # move one dir up and then go to image_data/frames/frame0000.jpg
        frame0000_path = os.path.abspath(os.path.join(curr_dir, '../image_data/frames/frame0000.jpg')) 
        frame = iio.imread(frame0000_path)
    ...
    frames_path = os.path.abspath(os.path.join(curr_dir, '../image_data/frames/*.jpg'))
    frames = glob.glob(frames_path)
    frames.sort()
    images = [np.array(iio.imread(f)) for f in frames]

    # extrinsic
    corners_path = frames_path = os.path.abspath(os.path.join(curr_dir, '../image_data/corners.npy'))
    ...
    drill_obj_path = os.path.abspath(os.path.join(curr_dir, '../image_data/models/drill.obj'))
    fuze_obj_path = os.path.abspath(os.path.join(curr_dir, '../image_data/models/fuze.obj'))
    drill_mesh = trimesh.load(drill_obj_path)
    fuze_mesh = trimesh.load(fuze_obj_path)
    ...
    print('Complete.')
    print('Saving GIF ...')
    with iio.get_writer('VR_res.gif', mode='I') as writer:
